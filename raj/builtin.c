...
void	built_in2(t_cmd *cmd, t_var *vars)
{
...
int	built_in1(t_cmd *cmd, t_var *vars)
{
	char *str;
...
//ignores extra spaces between characters
...
void ft_cd(t_cmd *cmd, t_var *vars)
{
	...
	pwdcd(old_pwd, sizeof(old_pwd)); //see ft_pwd
	dir = getenv("HOME");
	temp = ft_strjoin(dir, cmd.cmd[1]);
	new_pwd = temp;
	free(temp);
// If cmd has no arguments or argument is '~' then we go HOME
	//if (cmd->cmd[1] == NULL || (cmd->cmd[1][0] == '~' && cmd->cmd[1][1] == '\0'))
		//temp = dir;
		//no NEED to do anything can delete this if statement

//absolute chdir with ~
	else if (cmd->cmd != NULL && cmd->cmd[1][0] == '~' && cmd->cmd[1][1] == '//') //just one '/'
	{
		temp = ft_strjoin(dir, cmd.cmd[1] + 1);  //+ 1 to ignore ~  //temp is malloced by strjoin so we need free
		dir = temp;
		free(temp);
	}

	//seems its ok to type multiple args, only first arg matters so below removed
/*
