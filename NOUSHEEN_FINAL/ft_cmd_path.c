...
/*   Updated: 2022/06/17 16:15:20 by nali             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*	Checks if command is an absolute path by checking if it is accessible
	If yes, return path. Else, check PATHS variable in env variables for 
	command path*/
int	ft_set_cmd_path(t_cmd *cmd_node, t_var *vars)
{
	...
	{
		cmd_node->cmd_path = ft_strdup(cmd_node->cmd[0]);
		return (0);
	}
